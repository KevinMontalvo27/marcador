...
export const GET: RequestHandler = async ({ params }) => {
  ...
  if (statsError) {
    return json({ error: `Error al obtener las estadísticas: ${statsError.message}` }, { status: 500 });
  }
  ...
  if (equiposError) {
    return json({ error: `Error al obtener la información de los equipos: ${equiposError.message}` }, { status: 500 });
  }
 ...
 if (jugadoresError) {
   return json({ error: `Error al obtener la información de los jugadores: ${jugadoresError.message}` }, { status: 500 });
 }
